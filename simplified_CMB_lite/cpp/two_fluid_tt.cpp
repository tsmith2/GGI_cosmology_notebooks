...
class TwoFluidModel {
 public:
  ...
  void write_scale_summary(std::ostream &out) const {
    ...
    out << std::setprecision(8)
        ...
        << "  mode_threads = " << source_thread_count() << "\n";
  }
  ...
  Spectrum compute_spectrum() const {
    ...
    for (size_t i = 0; i < k_grid.size(); ++i) {
      ...
      swsrc[i] = sw_spline(k_grid[i]);
      dopv[i] = dop_spline(k_grid[i]);
    }
    ...
    for (size_t j = 0; j < ell.size(); ++j) {
      ...
      for (size_t i = 0; i < k_grid.size(); ++i) {
        ...
        const double src = std::sqrt(damping[i]) * primary_src;
 ...
 private:
  ...
  int source_thread_count() const {
    if (p.n_threads > 0) return std::max(1, std::min(p.n_threads, p.n_source));
    const unsigned int hw = std::thread::hardware_concurrency();
    return std::max(1, std::min(static_cast<int>(hw == 0 ? 1 : hw), p.n_source));
  }
  ...
  State rhs(double eta, const State &u, double k) const {
    ...
    du[1] = -theta_c + 3.0 * phi_p;
    du[2] = -Hconf * theta_gammab / (1.0 + R) +
  ...
  void solve_sources(std::vector<double> &k_values, std::vector<double> &sw,
    ...
    const int n_threads = source_thread_count();
    auto worker = [&]() {
      for (;;) {
        const int i = next.fetch_add(1);
        if (i >= p.n_source) break;
        const double grid =
            4.0 * static_cast<double>(i) / static_cast<double>(p.n_source - 1);
        k_values[i] = std::pow(10.0, grid) / etatoday;
        State u = integrate_source(k_values[i]);
        sw[i] = u[4] + u[0] / 4.0;
        dop[i] = -u[2] / k_values[i];
      }
    };
...
Params parse_args(int argc, char **argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    else if (key == "--n-threads") p.n_threads = std::stoi(need_value(key));

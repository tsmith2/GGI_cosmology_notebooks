#!/usr/bin/env python3
"""Teaching wrapper for the RECFAST/theta two-fluid TT solver with neutrinos.
...
def tt_spectrum(
    theta: np.ndarray, *, source_mode: str = "full", N_eff: float = N_EFF
) -> TTSpectrum:
    """Compute one TT spectrum from theta."""
    log_as, omega_cdm, omega_b, h, n_s = map(float, theta)
    server = TwoFluidCPPServer(source_mode=source_mode, N_eff=N_eff)
    try:
...
def source_spectra(theta: np.ndarray, *, N_eff: float = N_EFF) -> dict[str, TTSpectrum]:
    """Compute full, Sachs-Wolfe, Doppler, and ISW TT spectra."""
    modes = ["full", "sw", "doppler", "isw"]
    return {mode: tt_spectrum(theta, source_mode=mode, N_eff=N_eff) for mode in modes}
